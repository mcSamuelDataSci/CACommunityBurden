...
oshpdPlot <- function(myCounty = "California") {
  ...
  left_join(., fullCauseList, by = c("CAUSE" = "LABEL")) %>% filter(!is.na(CAUSE), Level == "lev2", county == myCounty, type != "Crude Hosp Rate") %>% filter(sex == "Total") %>%
    group_by(type) %>% mutate(nameOnly = forcats::fct_reorder(nameOnly, filter(., type == "Number of Hospitalizations") %>% pull(measure))) %>% 
