...
oshpdPlot <- function(myCounty = "California") {
  calculated_metrics %>% left_join(., fullCauseList, by = c("CAUSE" = "LABEL")) %>% filter(!is.na(CAUSE), Level == "lev2", county == myCounty) %>% filter(sex == "Total") %>%
    ...
    ggplot(., aes(x = nameOnly, y = measure)) + coord_flip() + geom_bar(stat = "identity") + facet_grid(. ~ type, scales = "free_x", labeller=labeller(type = hospMeasuresShort)) + scale_y_continuous(labels = scales::comma)
