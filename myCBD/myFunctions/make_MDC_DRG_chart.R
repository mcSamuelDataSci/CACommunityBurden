...
mdc_drg_plot <- function(myCounty = "CALIFORNIA", myOSHPDtype = "Number of Hospitalizations", mySex = "Total", myN = 10, myVar = "mdc" ) {
  ...
  #each type (nhosp, avgcharges, etc), which may not be the same for each.
  {
    plotData <-    mdc_drg %>%
      ...
      left_join(., hdCodes, by = c("mdc" = "V1")) %>%
      filter(county == myCounty, sex == mySex, diagnosis_var == myVar) %>%
      group_by(type) %>%
      mutate(V2 = forcats::fct_reorder(V2, filter(., type == myOSHPDtype) %>%
