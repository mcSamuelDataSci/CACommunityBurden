...
type == "Average Charges" ~ "Average Charges (in thousands)")) %>% left_join(., hdCodes, by = c("mdc" = "mdc_drg_codes"))
  ...
  plotData <- mdc_drg %>%
    mutate(type = factor(type, levels = c("Number of Hospitalizations","Total Charges (in thousands)","Average Charges (in thousands)"))) %>%
