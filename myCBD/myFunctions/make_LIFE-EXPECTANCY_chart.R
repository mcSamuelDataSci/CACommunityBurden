...
lifeTableCounty <- readRDS(paste0(LTplace,"/e0ciCounty.rds")) %>%
                     mutate(FIPSCounty=substr(GEOID,3,5))  %>%
                     left_join(geoMap,by="FIPSCounty") %>%
                     mutate(sex = str_to_title(sex))
 

lifeTableState <- readRDS(paste0(LTplace,"/e0ciState.rds")) %>%
...
lifeTableSet <- filter(lifeTableSet0,sex != "Total",race7 != "MR_NH")
...
# FIX MIN and MAX Year in global or other life tables function eventaully
 ...
 tplot<-
    ...
    ggplot(data=dat.1, aes(x=year, y=ex)) +
                geom_line(size=myLineSize,aes(color=race7,linetype=sex)) +
   #geom_point(shape = myPointShape,size=myPointSize)  +
        ylim(66, 90) +
       #  geom_line(data=dat.1,aes(x=year, y=exlow, group=sex,color=sex)) +
       # geom_line(data=dat.1,aes(x=year, y=exhigh, group=sex,color=sex)) +
   ...
   #  scale_colour_discrete(guide = 'none') +   # removed legend
     ...
     # geom_dl(aes(label = race7), method = list(dl.trans(x = x + myLineLabelSpace), "last.points", cex=myCex1, font="bold")) +
     # geom_dl(aes(label = race7), method = list(dl.trans(x = x - myLineLabelSpace), "first.points",cex=myCex1, font="bold"))  +
