...
# add to technical notes hte purposes and contents of each of these files
...
# makes dataframe of all possible combinations of county, year, CAUSE, and ageG for ----------------
...
ageCounty   <- merge(fullMatCounty,datAA1,by = c("county","year","sex","ageG","CAUSE"),all=TRUE)    # merge with "fullMatCounty"
ageCounty   <- merge(ageCounty,popCountySexTotAgeG,by = c("county","year","sex","ageG"),all=TRUE)   # merge with county age populations

ageCounty   <- merge(ageCounty,popStandard[,c("ageG","US2000POP")],by="ageG")
...
ageComm     <- merge(ageComm,popCommSexTotAgeG,by = c("comID","yearG","sex","ageG"),all=TRUE)   # merge with county age populations

ageComm     <- merge(ageComm,popStandard[,c("ageG","US2000POP")],by="ageG")
...
commAA <- ageComm %>% group_by(comID,yearG,sex,CAUSE) %>%
  summarize(oDeaths = sum(Ndeaths),
            ...
            aLCI    = ageadjust.direct(count=Ndeaths, pop=pop, rate = NULL, stdpop=US2000POP, conf.level = 0.95)[3]*100000,
            aUCI    = ageadjust.direct(count=Ndeaths, pop=pop, rate = NULL, stdpop=US2000POP, conf.level = 0.95)[4]*100000, 
            YLL.adj.rate   = ageadjust.direct(count=YLL, pop=pop, rate = NULL, stdpop=US2000POP, conf.level = 0.95)[2]*100000) 
...
ageTract     <- merge(ageTract,popTractSexTotAgeG,by = c("GEOID","yearG","sex","ageG"),all=TRUE)   # merge with county age populations
...
tractAA <- tractAA[!(tractAA$oDeaths==0),c("GEOID","yearG","CAUSE","sex","aRate","aLCI","aUCI","YLL.adj.rate")]  
...
# == Merge adjusted rates and final file organization and processing =================================

# merge adjusted rates 
...
##### !!!!! #########
datCounty$county[datCounty$county==STATE] <- toupper(STATE)


datCounty <- datCounty %>% select(-ageG,-stateRate) %>%
  order()
