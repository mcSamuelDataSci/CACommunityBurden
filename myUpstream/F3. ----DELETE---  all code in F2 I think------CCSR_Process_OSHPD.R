...
ccsMap <- read_excel(paste0(ccbUpstream,"upstreamInfo/ccs_dx_icd10cm_2019_1-BETA.xlsx"))  %>%
  ...
  ccsR_map  <- read_excel(paste0(ccbUpstream,"upstreamInfo/DXCCSR-vs-Beta-CCS-Comparison.xlsx"), sheet="ICD-10-CM Code Detail", guess_max= 70000) 
  
  ccsR_ICD10CM_to_CCSR1 <- ccsR_map %>% select(ICD10CM = "ICD-10-CM Code", CCSR1)
  
  
  ccsR_cause_link <- ccsR_map %>%
                      select(ccsrCode = CCSR1, ccsrName = `CCSR1 Label`, old_ccsName = `Beta Version CCS Category Description` ) %>%
                      unique()
  
  
  explore1 <- ccsR_map %>% filter(CCSR1 == "SYM017")
  explore2 <- ccsR_map %>% filter(`Beta Version CCS Category Description` == "Other perinatal conditions")
