...
cbd.link <- setDT(
	read_csv("cbdLinkCA.csv", col_types="icccc"),                   # !! update path
...
saveRDS(ccb.dead.tracts, file="dxTract.rds")		# GEOID date sex (char) age17 dx (for LT)
saveRDS(ccb.dead.mssa,   file="dxMSSA-ccb.rds")		# comID date sex (char) age17 dx (for LT)
saveRDS(ccb.dead.county, file="dxCounty.rds")		# GEOID date sex (char) age17 dx (for LT)
saveRDS(ccb.dead.state,  file="dxState.rds")		# GEOID date sex (char) age17 dx (for LT)
