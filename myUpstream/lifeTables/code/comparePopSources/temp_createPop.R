...
census_raceLink <- readxl::read_excel(paste0(fusionPlace, "jaspoWork/compareP3_pop/censusLink.xlsx"), sheet = "raceLink")

census_ageLink <- readxl::read_excel(paste0(fusionPlace, "jaspoWork/compareP3_pop/censusLink.xlsx"), sheet = "ageLink")

census_yearLink <- readxl::read_excel(paste0(fusionPlace, "jaspoWork/compareP3_pop/censusLink.xlsx"), sheet = "yearLink")


countyLink <- readxl::read_xlsx(paste0(ccbInfo, "County Codes to County Names Linkage.xlsx")) %>%
  select(countyName, FIPSCounty)


census <- read_csv(paste0(fusionPlace, "jaspoWork/compareP3_pop/censusPop_2019.csv")) %>%
  as.data.frame() %>%
  select(county = COUNTY, YEAR, AGEGRP, TOT_POP, TOT_POP, starts_with(c("NH", "H"))) %>%
  tidyr::pivot_longer(-(county:AGEGRP), names_to = "censusGroup", values_to = "population") %>%
  left_join(countyLink, by = c("county" = "FIPSCounty")) %>%
  left_join(census_ageLink, by = c("AGEGRP" = "census")) %>%
  left_join(census_yearLink, by = c("YEAR" = "census")) %>%
  


%>%
  left_join(censusLink, by = c("censusGroup" = "census")) %>%
  left_join(yearLink, by = "YEAR") %>%
  filter(!raceNameShort %in% c(NA, "Multi-Race", "NH/PI", "AI/AN", "NH"), !is.na(year), AGEGRP == 0) %>%
  group_by(county, year, raceNameShort) %>%
  summarise(population = sum(population)) %>%
  ungroup() %>%
  left_join(countyLink, by = c("county" = "FIPSCounty")) %>%
  mutate(type = "census") %>%
  select(year, county = countyName, raceNameShort, population, type)
