...
###MCS###
if (whichData == "real"){
  .dxtract	<- paste0(.securePath,"dxTract.rds") # output deaths by tract
  .dxmssa		<- paste0(.securePath,"dxMSSA.rds") # output deaths by mssa
  .dxcounty	<- paste0(.securePath,"dxCounty.rds") # output deaths by county
  .dxstate	<- paste0(.securePath,"dxState.rds") # output deaths by state
}
#########
...
.csum <- .dxcounty[year %in% 2015:2017,.(dx=sum(dx)),by=countyFIPS] # county level sum 
.tsum <- .dxtract[year %in% 2015:2017][
...
if (whichData=="fake") {
	.actual<-data.table(year=c(2015:2018),dx=c(233143,239006,242461,248118,245349,258694,261712,267556,268661)) # actual
	.sampled<-.dxstate[year>=2015 & year<=2017 & sex=="TOTAL",(dx=sum(dx))]         # sampled deaths statewide 2013-2017
	.factor<-.sampled/.actual[year %in% 2015:2017,.(sum(dx))][[1]]     				# ratio of sampled to actual deaths
...
# generate deaths time series, where each year represents a moving 5-year window of pooled deaths, e.g. 2015==2016-10
...
#ltdx.tract <- lapply(2015:2017,doExtractDx,d=.dxtract,nyrs=5,level="tract")
#ltdx.tract <- data.table(do.call(rbind,ltdx.tract))
ltdx.tract<-data.table(do.call(rbind,lapply(2015:2017,doExtract,d=.dxtract,nyrs=5,level="tract"))) 
ltdx.mssa<-data.table(do.call(rbind,lapply(2015:2017,doExtract,d=.dxmssa,nyrs=5,level="mssa"))) 
ltdx.county<-data.table(do.call(rbind,lapply(2015:2017,doExtract,d=.dxcounty,nyrs=3,level="county"))) 
ltdx.state<-data.table(do.call(rbind,lapply(2015:2017,doExtract,d=.dxstate,nyrs=1,level="state"))) 

## 3.7	summarize exposures (rename nx as dx temporarily to use the above subroutine doExtract)
.nxtract[,dx:=nx];
ltnx.tract<-data.table(do.call(rbind,lapply(2015:2017,doExtract,d=.nxtract,nyrs=5,level="tract"))) 
ltnx.tract[,nx:=dx]; ltnx.tract[,dx:=NULL]; .nxtract[,dx:=NULL]
.nxmssa[,dx:=nx];
ltnx.mssa<-data.table(do.call(rbind,lapply(2015:2017,doExtract,d=.nxmssa,nyrs=5,level="mssa"))) 
ltnx.mssa[,nx:=dx]; ltnx.mssa[,dx:=NULL]; .nxmssa[,dx:=NULL]
.nxcounty[,dx:=nx];
ltnx.county<-data.table(do.call(rbind,lapply(2015:2017,doExtract,d=.nxcounty,nyrs=3,level="county"))) 
ltnx.county[,nx:=dx]; ltnx.county[,dx:=NULL]; .nxcounty[,dx:=NULL]
.nxstate[,dx:=nx];
ltnx.state<-data.table(do.call(rbind,lapply(2015:2017,doExtract,d=.nxstate,nyrs=1,level="state"))) 
...
## - for time series, moving window, e.g. 2015 is from pooled 2006-10 ACS, 2011 from pooled 2007-11, etc.
...
## - E. Andreev and V. Shkolnikov. 2015. "Spreadsheet for calculation of confidence limits 
##   for any life table or healthy-life table quantity". MPIDR Technical Report 2015-005.
...
## - use to check any single LT for typical results, by it
