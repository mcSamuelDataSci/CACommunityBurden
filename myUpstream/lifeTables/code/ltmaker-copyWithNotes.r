...
# For county, we also want to do 1-year JASPO
# For state, we also want to do 3-year for race JASPO
...
raceLink <- read_xlsx(paste0(myStandards, "raceLink.xlsx")) %>%
  select(raceCode, CHSI, Ethan)

## 1.4 	links
#.ckey	  <- read_file(paste0(upPlace,"/upstreamInfo/census.api.key.txt")) # census API key
# Do not do anything with tract and MSSA for now JASPO
...
if (whichDeaths=="real") .deaths		<- paste0(mySecure,"/ccb_processed_deaths.RDS") # only use this JASPO
...
trt00mssa<-setDT(read.dta13(.trt00mssa)) # not use DT; regular data frame... dta is STATA JASPO
trt10mssa<-setDT(read.dta13(.trt10mssa))
mssacfips<-setDT(read.dta13(.mssacfips))

## 2.2 	load county name to county FIPS code maps
countycfips<-setDT(read.dta13(.countycfips)) # Use our standard fips JASPO
...
cbddeaths <- readRDS(.deaths); setDT(cbddeaths)

# will only use real, so dont need whichDeaths
...
if (whichDeaths %in% c("real","fake")) {
  ...
  dx.county[CHSI=="AIAN-NH",race7:="AIAN_NH"]
  dx.county[CHSI=="Asian-NH",race7:="ASIAN_NH"]
  dx.county[CHSI=="Black-NH",race7:="BLACK_NH"]
  dx.county[CHSI=="Hisp",race7:="HISPANIC"]
  dx.county[CHSI=="Multi-NH",race7:="MR_NH"]
  dx.county[CHSI=="NHPI-NH",race7:="NHPI_NH"]
  dx.county[CHSI=="White-NH",race7:="WHITE_NH"]
  dx.county[CHSI=="Other-NH",race7:="SOR_NH"]
...
if (whichDeaths %in% c("real","fake")) { 
  ...
  mx.mssa<-data.table(do.call(rbind,lapply(range,doExtract,dx=dx.mssa,nx=nxacs,nyrs=2,level="mssa")))[,nyrs:=5] # if mssa, 5 year ranges JASPO
}
## county
mx.county<-rbind( # combine 3-year TOTAL race, 5-year CHSI
  data.table(do.call(rbind,lapply(2001:2017,doExtract,dx=dx.county,nx=nx.county,nyrs=1,level="county")))[race7=="TOTAL"][,nyrs:=3], # If total race, 3 years JASPO
  data.table(do.call(rbind,lapply(2002:2016,doExtract,dx=dx.county,nx=nx.county,nyrs=2,level="county")))[,nyrs:=5] # if race, 5 years JASPO
)
## state
mx.state<-data.table(do.call(rbind,lapply(2000:2018,doExtract,dx=dx.state,nx=nx.state,nyrs=0,level="state")))[,nyrs:=1] # If state, 1 year JASPO

#XXX testing
mx.state$pDead = 100*mx.state$Dx / mx.state$Nx # quality Check - deaths / population
...
# Equivalent to filtering out JASPO
