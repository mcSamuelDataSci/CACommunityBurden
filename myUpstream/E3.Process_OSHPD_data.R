...
if(newData) {
  ...
  oshpd.PDD.17.0  <- read_sas(paste0(secure.location,"rawOSHPD/PDD/cdph_pdd_ssn2017.sas7bdat") ) ## SSN!
  oshpd.PDD.17    <- oshpd.PDD.17.0 %>% select(ourVars,contains("ccs_odiag"),contains("odiag")) %>% mutate(year=2017)
