...
tdat <- filter(tdat,year==2017)



tdat$pop <- ifelse(tdat$sex == "Male", -1*tdat$pop, tdat$pop)
...
n1 <- ggplot(tdat, aes(x = ageG, y = pop, fill = sex)) + 
  ...
  scale_y_continuous(breaks = seq(-3000000, 3000000, 100000)) + 
  coord_flip() + 
...
p1 <- ggplot( tdat, aes( x = year, y = pop ) ) + 
