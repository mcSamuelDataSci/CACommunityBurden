...
cbdMap <- function(myLHJ     = "Alameda", myCause     = "A01",   myMeasure = "YLLper",       myYear = 2015,
  ...
  countyPop <- datCounty %>% filter(year == 2017,sex=="Total", causeCode=="0") %>%
  ...
  if (myGeo == "County"){
    dat.State  <- filter(datCounty,(myYear>= 2012 & myYear <= 2017),sex==mySex, causeCode==myCause)  %>% mutate(geoLab = county)  
    dat.1      <- filter(datCounty,year==myYear,sex==mySex, causeCode==myCause)  %>% mutate(geoLab = county) %>% select(-population)
...
mapX <-  tm_shape(map.1) + tm_polygons(col=myMeasure, palette = myPal, 
                    ...
                    tm_layout(frame=F,
                              main.title= myTit,
                              main.title.size = 1.5,
                              main.title.color = "darkblue", #myTitleColor,
                              title.color = "yellow",
                              fontface = 2,
