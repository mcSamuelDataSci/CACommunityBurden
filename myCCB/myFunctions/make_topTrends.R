...
topCauses_trends <- function(
  ...
  plot_df <- datCounty %>%
    ...
    select(year, Level, county, causeCode, causeNameShort, causeNameShort_left_dl, topLevCode, topLevName, measure)
  ...
  if (myLev == "lev1") {
    ...
    tPlot <- ggplot(tDat, aes(x = year, y = measure, color = causeNameShort) )  +
      ...
      geom_dl(aes(label = causeNameShort), method = list(dl.trans(x = x + .1), "last.points", cex = 1.3, 'bumpup')) +   # , 'last.bumpup'
      geom_dl(aes(label = causeNameShort_left_dl), method = list(dl.trans(x = x - .1), "first.points", cex = 1.3, 'bumpup', hjust = 1)) +
      theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
