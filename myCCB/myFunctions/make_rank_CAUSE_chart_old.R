...
rankCause <- function(myCounty = "Los Angeles", myMeasure = "Number of deaths", myYear = 2018, mySex = "Total", myLev = "lev2", myN = 10){ 
  ...
  #ordering dataset, replaces short names with full names
  temp <- datCounty %>% gather(key = "type", value = "measure", Ndeaths,YLLper,aRate,mean.age,SMR)  %>% # replace with pivot longer
    select(year,sex,Level,causeCode,county,type,measure) %>%
    mutate(type = factor(type,levels= dM_short))    %>% #orders factor
    mutate(type = plyr::revalue(type, dMNames_short))    %>% #replaces values with full name labels; changed dmRevalue to dmNames_short
  ...
  temp_N_cause <- temp %>%
    filter(sex == mySex, Level %in% myLev, year == myYear, county == myCounty) %>%
    group_by(type) %>% arrange(desc(measure)) %>% dplyr::slice(1:myN) %>% #this selects the top N rows for myOSHPDtype
    filter(type == myMeasure) %>% ungroup() %>% pull(causeCode)
  
  #creates dataframe with data only for CAUSEs from temp_N_cause, i.e. the top N CAUSES for the specified temp_N_cause        
  plotData <-     temp %>%
    filter(!is.na(causeCode), Level %in% myLev, county == myCounty, causeCode %in% temp_N_cause) %>% 
    filter(sex == mySex, year == myYear) %>%
    group_by(type)    %>% 
    mutate(nameOnly = forcats::fct_reorder(causeName, filter(., type == myMeasure)  %>% 
                                             pull(measure)))  
  ...
  if (myCounty != "CALIFORNIA") {
    ...
    xtemp <- xtemp +
      ...
      geom_hline(data = data.frame(yint = 1, type = "Standard Mortality Ratio"), aes(yintercept = yint, color="State Average"), linetype="dashed", size = 1) +#adds SMR line only to SMR facet
