...
# rmarkdown::render("State of Health - Web Share Version.Rmd")

 knitr::opts_chunk$set(echo = FALSE, warnings = FALSE, message = FALSE, include=TRUE) 
...
ggplot(t.dat,aes(x=topLev,y=Ndeaths,label=shortName,
...
```{r}
# Bigger fig.width
include_graphics("ourImages/Black-White Disparity A.PNG")
include_graphics("ourImages/Black-White Disparity B.PNG")
...
pA <- cowplot::plot_grid(pA,NULL, pB, NULL, pC,ncol=1,rel_heights = c(1,0.05, 1, 0.05, 1))
...
cowplot::plot_grid(p1,p2,p3,p4,p5,ncol=2, nrow = 3)
...
ggplot(ccbChange_08_18_Decrease,aes(x=reorder(causeName,-change),y=change,fill=topLev,label=paste(Ndeaths_2008,"to",Ndeaths_2018))) + 
                 geom_bar(stat= "identity") + coord_flip() +
                 geom_text(size=3,y=-.02,hjust=1) + #position=dodge,size=3)+
...
ggplot(ccbChange_08_18,aes(x=reorder(causeName,change),y=change,fill=topLev,label=paste(Ndeaths_2008,"to",Ndeaths_2018))) +  
                 geom_bar(stat= "identity") + coord_flip() +
                   geom_text(size=3,y=.02,hjust=0) + #position=dodge,size=3)+
...
ggplot(hosp,aes(x=reorder(stringr::str_wrap(ccsName, width = 25),measure) ,y = measure)) + 
                 geom_bar(stat= "identity") + coord_flip() +
...
ggplot(hosp,aes(x=reorder(stringr::str_wrap(ccsName, width = 25),measure) ,y = measure)) + 
                 geom_bar(stat= "identity") + coord_flip() +
                 labs(x="Cause Group",y="total charges for hospitalizations")+
                 scale_y_continuous(label=comma)
...
cowplot::plot_grid(p1,NULL, p2,NULL, p3,NULL, p4,NULL, p5,ncol=1, rel_heights = c(1, 0.05, 1, 0.05, 1, 0.05, 1, 0.05, 1))
