...
.t.2 <- work.dat  %>%  select(CAUSE,Ndeaths,raceCode) %>%          # N BY RACE + N RANK  (8)
...
.t.3 <- work.dat %>%  select(CAUSE,aRate,raceCode) %>%           # ADJUSTED RATE BY RACE + RANK + RATIO (12)
...
t.X <- .t.3 %>% 
          mutate(maxRatio = max(Aratio,Bratio,Hratio,Wratio),
